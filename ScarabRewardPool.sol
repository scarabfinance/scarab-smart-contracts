...
// Note that this pool has no minter key of SCARAB (rewards).
// Instead, the governance will call SCARAB distributeReward method and send reward to this pool at the beginning.
contract ScarabRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SCARABs to distribute in the pool.
        uint256 lastRewardTime; // Last time that SCARABs distribution occurred.
        uint256 accTombPerShare; // Accumulated SCARABs per share, times 1e18. See below.
    ...
    // The time when SCARAB mining starts.
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "ScarabRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "ScarabRewardPool: existing pool?");
        }
    }
    ...
    // Safe tomb transfer function, just in case if rounding error causes pool to not have enough SCARABs.
